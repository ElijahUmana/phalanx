...
2. You POST to `http://localhost:3000/api/scan` with `{"repoUrl": "<url>"}` and get back a `scanId`.
3. Open a Server-Sent Events stream at `http://localhost:3000/api/status?scanId=<scanId>`.
...
Do not narrate every intermediate event. The Phalanx dashboard at `http://localhost:3000/dashboard` is the full visual; link the user there if they want to watch live.
